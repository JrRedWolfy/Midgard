...
CREATE TABLE DEPARTAMENTO_USUARIO ( /* PERTENECER */
  ...
  CONSTRAINT FK2 FOREIGN KEY (username) references USUARIO (username) on delete restrict on update cascade,
  CONSTRAINT FK3 FOREIGN KEY (id_departamento) references DEPARTAMENTO (id_departamento) on delete restrict on update cascade
);
...
CREATE TABLE PUBLICACION (
  ...
  id int, 
  CONSTRAINT FK4 FOREIGN KEY (escritor) references USUARIO (username) on delete restrict on update cascade,
...
CREATE TABLE PANTALLA_PUBLICACION ( /* ESTAR */
  ...
  CONSTRAINT FK7 FOREIGN KEY (id_publicacion) references PUBLICACION (id_publicacion) on delete restrict on update cascade,
  CONSTRAINT FK8 FOREIGN KEY (ip_pantalla) references PANTALLA (ip) on delete restrict on update cascade
);
